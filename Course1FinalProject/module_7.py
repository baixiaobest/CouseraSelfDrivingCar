...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        time_history  = [0]
        speed_history = [0]
        reference_speed_history = [0]
        throttle_history = [0]
        cross_track_error_history = [0]
        ...
        #################################
        plt.ion()
        control_plot_fig, ax = plt.subplots(nrows=3, ncols=1)
        forward_speed_line, = ax[0].plot([], [], color='k')
        reference_speed_line, = ax[0].plot([], [], color='b')
        throttle_history_line, = ax[1].plot([], [])
        cross_track_error_line, = ax[2].plot([], [])
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            reference_speed_history.append(controller._desired_speed)
            ...
            if skip_first_frame and frame == 0:
                ...
                cross_track_error_history.append(controller.get_cross_track_error())

                forward_speed_line.set_data(time_history, speed_history)
                reference_speed_line.set_data(time_history, reference_speed_history)
                throttle_history_line.set_data(time_history, throttle_history)
                cross_track_error_line.set_data(time_history, cross_track_error_history)

                for i in range(len(ax)):
                    ax[i].relim()
                    ax[i].autoscale_view()
                # We need to draw *and* flush
                control_plot_fig.canvas.draw()
                control_plot_fig.canvas.flush_events()
