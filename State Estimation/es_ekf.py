...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    sensor_covar = np.identity(3) * sensor_var

    # 3.1 Compute Kalman Gain
    K = p_cov_check @ h_jac.T @ inv(h_jac @ p_cov_check @ h_jac.T + sensor_covar)
...
def find_closest_measurement(t, times, start_idx=0):
    idx = -1
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_k_1 = p_est[k - 1]
    v_k_1 = v_est[k - 1]
    q_k_1 = Quaternion(q_est[k - 1, 0], q_est[k - 1, 1], q_est[k - 1, 2], q_est[k - 1, 3])
    Cns = q_k_1.to_mat()
    ...
    lidar_idx = find_closest_measurement(curr_t, lidar.t, max(last_lidar_idx, 0))
    no_sensor_available = True

    if gnss_idx != -1 and gnss_idx > last_gnss_idx:
        p_check, v_check, q_check, p_cov_check = \
            measurement_update(var_gnss, p_cov_check, gnss.data[gnss_idx], p_check, v_check, q_check)
        last_gnss_idx = gnss_idx
        no_sensor_available = False

    if lidar_idx != -1 and lidar_idx > last_lidar_idx:
        p_check, v_check, q_check, p_cov_check = \
            measurement_update(var_lidar, p_cov_check, lidar.data[lidar_idx], p_check, v_check, q_check)
